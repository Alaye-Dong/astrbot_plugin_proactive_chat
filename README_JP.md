...
>
> 開発期間：累計 37 日（メインプラグイン部分）
>
> 累計工数：約 215 時間（メインプラグイン部分）
>
> 本リポジトリの作成：累計 3 日、約 22 時間
>
> 使用した大規模言語モデル：Gemini-2.5-Pro、Kimi For Coding、Gemini-3.0 Flash/Pro (With RooCode in VSCode)
>
...
**Q: エラーメッセージ ApiNotAvailable？**

> **A**: 現在、既知のすべての情報は、`ApiNotAvailable` がプラグイン自体のバグではなく、基盤となる OneBot アダプターと物理ロボット（NapCat/Lagrange など）の間の接続異常であることを示しています。
> 一般的な原因としては、ロボットがオフラインになっているのにプラットフォームが検出していない、ハートビート間隔が長すぎてルーター/ファイアウォールによって切断されている、バックグラウンドにゾンビ/未接続のプラットフォーム設定項目がある、さらにはブラウザの省電力モードが WebSocket ページに影響を与えている（Edge の事例がありますが、唯一の原因ではありません）などが挙げられます。
>
> この問題は、AstrBot のバージョン、デプロイ方法（Windows/Docker）、ネットワーク環境によって動作が異なり、安定して再現することができません。一部のユーザーはバージョンを戻した後に問題が解消され、他のユーザーでは再発することから、環境要因が支配的であることを示しています。
> 以下の調査を重点的に行うことをお勧めします：
>
> 1. OneBot クライアントのログを確認し、頻繁な Close/Reconnect やオフラインのプロンプトがないかチェックしてください。
> 2. すべてのプラットフォーム設定項目を確認し、冗長な、または接続されていない「ゴーストプラットフォーム」がないことを確認してください。
> 3. ハートビート設定を短く（例：30秒）して、接続をアクティブに保つことをお勧めします。
> 4. ネットワーク環境を調査し、長い接続に影響を与えるファイアウォール、NAT、ルーターの省電力などの設定がないか確認してください。
> 5. ブラウザを使用して WebSocket を管理している場合は、省電力モードをオフにするか、ブラウザを切り替えてみてください。
> 6. AstrBot や NapCat などのサービスを再起動してください。Bot がサーバーにデプロイされている場合は、必要に応じてサーバーを再起動してください。
>
> エラーが発生した際の詳細な OneBot クライアントログを取得したり、異なるネットワーク環境/デバイス/ブラウザで比較テストを行ったりすることが、問題のさらなる特定に役立つ可能性があります。
> この問題の本質は「物理ロボットとプラットフォーム間の接続の不安定さ」です。コードレベルでポーリングとフォールトトレランスが実装されており、残りは環境の調査によってのみチェックできます。
