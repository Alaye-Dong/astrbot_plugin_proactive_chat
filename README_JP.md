...
> 一般的な原因としては、ロボットがオフラインになっているのにプラットフォームが検出していない、ハートビート間隔が長すぎてルーター/ファイアウォールによって切断されている、バックグラウンドにゾンビ/未接続のプラットフォーム構成項目がある、さらにはブラウザの省電力モードが WebSocket ページに影響を与えている（Edge の事例がありますが、唯一の原因ではありません）などが挙げられます。
>
> この問題は、AstrBot のバージョン、デプロイ方法（Windows/Docker）、ネットワーク環境によって動作が異なり、安定して再現することができません。一部のユーザーがバージョンを戻した後に正常になり、他のユーザーでは再発することから、環境要因が支配的であることを示しています。
> 以下の調査を重点的に行うことをお勧めします：
>
> 1. OneBot クライアントのログを確認し、頻繁な Close/Reconnect やオフラインのプロンプトがないかチェックしてください。
> 2. すべてのプラットフォーム構成項目を確認し、冗長な、または接続されていない「ゴーストプラットフォーム」がないことを確認してください。
