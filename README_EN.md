...
- [🚧 Known Limitations of the Latest Version](#-known-limitations)
- [❓ FAQ](#-faq)
...
## 🚧 Known Limitations of the Latest Version

- **Multi-instance not supported**: Currently only adapted for a single instance. If you have multiple bots, the additional bots will not be able to perform proactive messaging correctly, and it may affect the plugin's selection of the bot instance to send proactive messages. If you encounter a situation where an unexpected Bot sends you proactive messages and the plugin cannot correct itself, please try deleting the persistent data generated by the plugin, and then chat again with the Bot you want to send proactive messages.
